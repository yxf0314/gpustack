...
def test_manual_distributed_enabled(case_name, env, expected):
    # No gpu_selector constraint: gateway opt-out is independent of GPU selection.
    model = make_model(
        gpus_per_replica=8,
        gpu_ids=None,
        backend=BackendEnum.VLLM.value,
    ...
    assert manual_distributed_enabled(model) is expected, f"case '{case_name}' failed"
