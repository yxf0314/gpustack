...
- **Replicas can be adjusted in the plan.** With hand-picked GPUs, check that `gpu_selector` still holds enough of them for the new count, exactly as you would when editing the deployment. With scheduled scaling enabled the count sets `scaling_schedule.baseline_replicas`, since the schedule owns the replica count.
