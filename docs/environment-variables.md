...
| `GPUSTACK_MANUAL_DISTRIBUTED`                             | Switches the model to manual distributed deployment, a superset of `GPUSTACK_SKIP_GPU_COUNT_CHECK`. When enabled it (1) skips the GPU-count-vs-world-size check (same effect, only when GPUs are manually selected) and (2) skips this model's gateway registration — no upstream is registered and its route destinations stay empty. Use when you wire N independent single-node deployments into one vLLM data-parallel cluster yourself (via `--data-parallel-address`, etc.) and manage the load balancer / gateway externally. Recommended together with `enable_model_route=false`. | (empty) | Model      |
