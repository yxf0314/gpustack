...
def _select_serving_worker_id(instance: ModelInstance, model: Model) -> int:
    ...
    # Only round-robin onto subordinates that are RUNNING; a subordinate marked
    # ERROR (e.g. by the inference health check) must not receive traffic until
    # the instance recovers. The leader is always included.
    worker_ids = [instance.worker_id] + [
        ...
        and subordinate_worker.state == ModelInstanceStateEnum.RUNNING
    ]
