...
async def estimate_model_vram(model: Model, token: Optional[str] = None) -> int:
    """
    Estimate the vram requirement in bytes heuristically for SGLang.
    This is the minimum requirement to help us decide how many GPUs are needed for the model.

    SGLang has similar memory requirements to vLLM but with different memory management.
    Formula: VRAM = WEIGHT * 1.2 + RESERVED_FOOTPRINT

    SGLang uses --mem-fraction-static (default 0.9) for memory management instead of
    --gpu-memory-utilization.
    """
    if model.env and 'GPUSTACK_MODEL_VRAM_CLAIM' in model.env:
        return int(model.env['GPUSTACK_MODEL_VRAM_CLAIM'])

    # SGLang memory overhead - similar to vLLM but with different allocation strategy
    # SGLang uses static memory fraction for KV cache pool
    framework_overhead = (
        2 * 1024**3
...
def parse_model_size_by_name(model_name: str) -> int:
    """Parse model size from model name for SGLang models."""
    match = re.search(r'(\d+(?:\.\d+)?)B', model_name, re.IGNORECASE)
    if match:
        size_str = match.group(1)
        size_float = float(size_str)
        return int(size_float * 1000**3)
    return 0


def get_local_model_weight_size(local_path: str) -> int:
    """Get the weight size of a local model for SGLang."""
    total_size = 0
    supported_extensions = ['.bin', '.safetensors', '.pt', '.pth']
    ...
    try:
        ...
        logger.warning(f"Error calculating local model size: {e}")
        return 0
...
class SGLangResourceFitSelector(ScheduleCandidatesSelector):
    def __init__(
        ...
        enable_mixed_chunk = (
            str_to_bool(enable_mixed_chunk_param)
        ...
        if pp_size and int(pp_size) > 1:
            ...
            disable_overlap_schedule = (
                str_to_bool(disable_overlap_schedule_param)
    ...
    async def manually_select_single_worker_multi_gpu_candidates(
        ...
        for gpu_idx in gpu_indexes:
            allocatable_varm = allocatable.vram.get(gpu_idx, 0)
            if allocatable_varm < vram_per_gpu:
                overcommit = True
            elif (
                allocatable_varm / worker.status.gpu_devices[gpu_idx].memory.total
                < self._mem_fraction_static
            ):
                overcommit = True
